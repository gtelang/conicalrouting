...
# This modelling software is widely used by the operations research community. It has default solvers included 
# but can also interact with CPLEX and many other powerful solvers. 
import pulp
...
#-------------------------------------------------------------------------------------------------
def min_length_intprog(G, start_node_idx, target_node_idx, lookahead_lb, shiftvals, W, makeS_p=False, lpfilename="lookahead.lp"):
     """
     ...
     #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
     shift = shiftvals[0] 
     for anum in range(0,W+1): 
           print Fore.YELLOW, ".....Generating Type [D] (lookahead) constraint for half-plane ",  anum, "<---->", anum+shift,  Style.RESET_ALL

           halfplane_edges = [(u,v) for (u,v) in list(G.edges()) if (u[0] >= anum) and v[0]<=(anum + shift)   ] # a halfplane edge is a graph edge whose tail and head lies in the halfplane. By construction of the cones we can just use arm indices to decide this. 
           prob           += pulp.lpSum( [ edge_costs[hpe]*edge_vars[hpe] for hpe in halfplane_edges] ) >= lookahead_lb , \
                             ("String in half-plane [" + str(anum) + '<--->' + str(anum+shift) + "] should have length >= " +\
                              str(round(lookahead_lb,4)))
           
     # An hplane consists of a pair of arms that share the same basepoint and opposite direction.
...
#-------------------------------------------------------------------------------------------------
def conical_routing(fig, ax, box, dx, eps, W, lookahead_lb, lookaheadmethod=min_length_intprog):
     ...
     #print "Shortest path nodes are "
     #for idx in spathnodeids:
     #     print Fore.CYAN, idx, G.node[idx]['cood'] , Style.RESET_ALL

     print Fore.GREEN, "Shortest path length (RED) is ", round(pathlength(G,spathnodeids),4), Style.RESET_ALL
     ax.set_xlabel("Shortest Path length (RED): " + str(round(pathlength(G,spathnodeids),4)), fontsize=13)
     #......................................................................................................
     # Solve the integer programming model here. 
     #......................................................................................................
     print("==================> Solving the integer/linear program here <====================")
     start_time      = time.time() 
     lookpathnodeids = lookaheadmethod(G,start_node_idx, target_node_idx, lookahead_lb=lookahead_lb, shiftvals=shiftvals, W=W)
     end_time        = time.time() 
     time_elapsed    = end_time - start_time
     print "Time taken for lookaheadmethod function to run : ", round(time_elapsed,4) , " seconds"
     print Fore.MAGENTA, "\n.............. \n The path is \n............\n"
     printlist(lookpathnodeids)
...
#--------------------------------------------------------------------------------------------
def draw_graph_edges(fig, ax, G, alpha=0.1, markendpts_p=False):
     """ Render all the graph edges by means of rendering all their associated edge segments. 
     """
     
     for u,v in G.edges():
          G.edges[u, v]['edgeseg'].draw(fig,ax, alpha=alpha, markendpts_p=markendpts_p) 
...
#-------------------------------------------------------------------------------------------
def example_environment_simplest ():
     """ Exactly one mite and no tite
     """
     ...
     mtips  = list(map(np.asarray, [ [0.5,0.5] ]))
     mites = []                     
     ...
     box = Box(xleft,xright,ydown,yup, mites, start=start, target=target);
     return box


#-------------------------------------------------------------------------------------------
def example_environment_simpler ():
     """ One mite and tite each
     """
     start  = np.asarray([0,0])
     ...
     yup    = 1
     #stalagmites (mites go up)                                                     
     mtips  = list(map(np.asarray, [ [0.3,0.5] ]))
     mites = []                     
     for i in range(len(mtips)):
         p = np.asarray([mtips[i][0],ydown])
         q = mtips[i]
     ...
     ttips  = list(map(np.asarray, [(0.7,0.2)]))
     tites = []
...
def example_environment ():
     """ Bunch of mites and tites. 
     """
     start  = np.asarray([0,0])
     target = np.asarray([3,1])
     
     # Be careful! the following box constants should be set independently of the position of start and target
     # otherwise the placement of the stalactite and stalagmite will get affected badly, leading to code crashes. 
     # This led me to a nasty bug hunt, where I was setting xleft  to be start[0]. cright to be target[0] etc. 
     xleft  = 0
     xright = 3
     ydown  = 0
     yup    = 1
...
def main():
     ...
     box     = example_environment_simplest()    # wedge shift is 6 for above parameters.
     #box    = example_environment_simpler()     # wedge shift is 6 at both obstacle tips for same ditto parameters.
     #box    = example_environment() # dont know about wedge shift...we will do this later after the first two are knocked off.
     ...
     lookahead_curve = conical_routing(fig, ax, box, dx=dx, eps=eps, W=W, lookahead_lb = lookahead_lb)
     plt.show()
#-----------------------------------------------------------------------------    
